   ...
   if (isTouchDevice()) {
     ...
     if (
       ...
       currentDrop.insertAdjacentHTML(
         "afterbegin",
         `<img src="flags/${currentElement.id}.png">`
       );
     ...
     if (draggedElementData === droppableElementData) {
       ...
       e.target.insertAdjacentHTML(
         "afterbegin",
         `<img src="flags/${draggedElementData}.png">`
       );
 ...
 const creator = () => {
   ...
   let randomData = [];
   let correctAnswer = randomValueGenerator(); // Select the correct answer first
   randomData.push(correctAnswer);

   // Select other random countries until we have 3 unique options
   ...
   // Create drop points for each country option
